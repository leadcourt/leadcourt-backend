...
exports.zohoCallback = async (req, res) => {
  try {
    ...
    const Zoho = await ZohoModel.findByIdAndUpdate(uid, { 
    ...
    return res.redirect(`${process.env.FRONTEND_BASE}/zoho/connected?status=success`);
    // Or respond with JSON:
    // return res.json({ success: true, user });
  } catch (err) {
    console.error('Zoho callback error', err.response?.data || err.message);
    return res.redirect(`${process.env.FRONTEND_BASE}/zoho/connected?status=error`);
...
exports.exportToZoho = async (req, res) => {
  try {
    ...
    const result = await zohoClient.createZohoRecord(accessToken, 'Leads', records);
    return res.json({ success: true, result });
  } catch (err) {
    console.error('Export to Zoho error', err.response?.data || err.message);
    return res.status(500).json({ message: 'Export failed', error: err.response?.data || err.message });
  }
};
